...
function ChartTooltipContent({
  ...
  nameKey,
  labelKey,
}: Omit<ChartTooltipContentProps, "label"> & { label?: any }) {
      ...
      <div className="grid gap-1.5">
        {payload.map((item, index) => {
          ...
          const indicatorColor =
            color || (item as any)?.payload?.fill || (item as any)?.color || (item as any)?.fill;
          ...
          return (
            ...
            >
              {formatter && (item as any)?.value !== undefined && item.name ? (
                formatter((item as any).value, item.name, item as any, index, (item as any).payload)
              ) : (
                <>
                  {itemConfig?.icon ? (
                    <itemConfig.icon />
